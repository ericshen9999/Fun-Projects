        ...
        for j in mesh['adj'][box_pop]:
            ...
            if direction == "forward":
                if j not in boxesF:
                    ...
                    boxesF[j] = (distance_pop + distance_new, (box_pop, position_new))
                    heappush(queue, (distance_end + distance_pop + distance_new, distance_pop + distance_new, j, position_new, "forward"))
            else:
                if j not in boxesB:
                    distance_new, position_new = find_closest(j, position_pop, box_pop)
                    distance_end = find_distance(position_new, source_point)
                    boxesB[j] = (distance_pop + distance_new, (box_pop, position_new))
                    heappush(queue, (distance_end + distance_pop + distance_new, distance_pop + distance_new, j, position_new, "backwards"))
        #Error catching
...
def find_closest(box1, curPos, box_pop):
    output = []
    #find where 2 boxes touch [Not Done Yet]
    if box_pop[0] >= box1[0] and box_pop[0] <= box1[1]:
        if box_pop[2] == box1[3]:
            heappush(output,(find_distance((box_pop[0],box_pop[2]),curPos), (box_pop[0],box_pop[2])))
        if box_pop[3] == box1[2]:
            heappush(output,(find_distance((box_pop[0],box_pop[3]),curPos), (box_pop[0],box_pop[3])))
    if box_pop[1] >= box1[0] and box_pop[1] <= box1[1]:
        if box_pop[2] == box1[3]:
            heappush(output,(find_distance((box_pop[1],box_pop[2]),curPos), (box_pop[1],box_pop[2])))
        if box_pop[3] == box1[2]:
            heappush(output,(find_distance((box_pop[1],box_pop[3]),curPos), (box_pop[1],box_pop[3])))
    ...
    if box1[0] >= box_pop[0] and box1[0] <= box_pop[1]:
        if box_pop[2] == box1[3]:
            heappush(output,(find_distance((box1[0],box1[3]),curPos), (box1[0],box1[3])))
        if box_pop[3] == box1[2]:
            heappush(output,(find_distance((box1[0],box1[2]),curPos), (box1[0],box1[2])))
    if box1[1] >= box_pop[0] and box1[1] <= box_pop[1]:
        if box_pop[2] == box1[3]:
            heappush(output,(find_distance((box1[1],box1[3]),curPos), (box1[1],box1[3])))
        if box_pop[3] == box1[2]:
            heappush(output,(find_distance((box1[1],box1[2]),curPos), (box1[1],box1[2])))
